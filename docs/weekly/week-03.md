...
- **남은 확인**: qwen3 임베딩의 k-NN 분류 — k-NN은 임베딩 모델 의존적이므로(본 문서
  [C] 참고) slim 모드 성능이 같이 바뀐다. 85%+면 qwen3를 새 기본값으로 제안한다.
- 교체 확정 시 작업: `OLLAMA_EMBED_MODEL` 기본값 한 줄 + 재색인. ADR-0002 §2 결정의
  개정이므로 ADR 갱신과 팀 공유가 따른다.
